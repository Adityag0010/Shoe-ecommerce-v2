...
const ShoeDetail = () => {
    ...
    useEffect(() => {
        const fetchShoes = async () => {
            ...
            setLoading(true)
            try {
                const response = await axios.get(`https://shoe-ecommerce-v2.onrender.com/api/v1/products/${id}`);
    ...
    const fetchRelatedShoe = async () => {
        ...
        const response = await axios.get(
            "https://shoe-ecommerce-v2.onrender.com/api/v1/products/filter/related",
            {
            params: {
                gender: shoe?.for,
    ...
    const handleAddToCart = async () => {
        ...
        try {
            await axios.post(
                "https://shoe-ecommerce-v2.onrender.com/api/v1/cart", 
                { productId : shoe._id }, 
                { withCredentials: true }
            );
    ...
    return (
        <>
            <Helmet>
                <title>{`${shoe?.name || 'Loading...'} | UrbanSole`}</title>
                <meta name="description" content={shoe?.description || `Buy ${shoe?.name} from UrbanSole.`} />
                <meta property="og:title" content={`${shoe?.name} | UrbanSole`} />
